// Command ingest-zillow runs the Zillow ingester and writes the result to
// go/.cache/zillow.json.
package main
...
func main() {
	...
	client := httpclient.New(httpclient.Options{Timeout: 120 * time.Second, MaxRetries: 3})
	src := zillow.New(client)
	...
	cachePath := filepath.Join(".cache", "zillow.json")
