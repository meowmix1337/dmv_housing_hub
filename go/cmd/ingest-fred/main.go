// Command ingest-fred runs the FRED ingester and writes the result to
// go/.cache/fred.json. Loads .env from project root, then go/.env as override.
package main
...
func main() {
	...
	// Project-root .env first, then go/.env as override.
	...
	cachePath := filepath.Join(".cache", "fred.json")
